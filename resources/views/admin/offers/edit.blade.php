<x-layouts.admin :title="__('web.edit')">
    <div class="max-w-2xl">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-800">{{ __('web.edit') }} — {{ app()->getLocale() === 'ar' ? $offer->title_ar : $offer->title_en }}</h1>
        </div>
        <form action="{{ route('admin.offers.update', $offer) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="discount_type" required class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-rose-gold">
                        <option value="percentage" {{ old('discount_type', $offer->discount_type?->value) === 'percentage' ? 'selected' : '' }}>{{ __('web.percentage') }}</option>
                        <option value="fixed" {{ old('discount_type', $offer->discount_type?->value) === 'fixed' ? 'selected' : '' }}>{{ __('web.fixed') }}</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('web.discount_value') }} *</label>
                    <input type="number" name="discount_value" value="{{ old('discount_value', $offer->discount_value) }}" step="0.01" min="0" required
                           class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-rose-gold">
                </div>
                <div>
                    ...
                    @foreach($services as $svc)
                        <label class="flex items-center gap-2 text-sm cursor-pointer">
                            <input type="checkbox" name="service_ids[]" value="{{ $svc->id }}"
                                   {{ in_array($svc->id, old('service_ids', $offer->services->pluck('id')->toArray())) ? 'checked' : '' }}
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_active" value="1"
                       {{ old('is_active', $offer->is_active) ? 'checked' : '' }} class="rounded text-rose-gold">
